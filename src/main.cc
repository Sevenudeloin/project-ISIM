...
int main()
{
    ...
    algorithms::array2D height_map(height_map_size, std::vector<float>(height_map_size, 0.0f));
    ...
    algorithms::diamond_square(height_map, 0.2);
    ...
    for (int j = 0; j < image_height; j++) {
        for (int i = 0; i < image_width; i++) {
            ...
            if (height < 0.0f || height > 1.0f) {
                std::clog << "Error: height out of bounds: " << height << std::endl;
                return 1;
            }
