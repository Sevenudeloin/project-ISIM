    ...
    while ((opt = getopt(argc, argv, "o:s:t:h")) != -1) {
        switch (opt) {
            ...
            case 'h':
                ...
                break;
            default:
                std::cerr << "Usage: " << argv[0] << " [-o <output_filename>] [-s <width>x<height>] [-t <scene_type>] [-h]" << std::endl;
                return 1;
        }
    }

    if (show_help) {
        std::cout << "Usage: " << argv[0] << " [-o <output_filename>] [-s <width>x<height>] [-t <scene_type>] [-h]" << std::endl;
        ...
        std::cout << "  -t <scene_type>       Specify the scene type (available: test, ocean, island), (default is test)" << std::endl;
